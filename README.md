...
### Why 2 SQS Queues (Not SNS)?

This project uses **2 dedicated SQS queues** (payment and shipping) instead of SNS:

**Rationale:**
- **Assessment Context:** Demonstrates pragmatic engineering judgment - choosing the right tool without over-engineering
- **Decoupled Processing:** Each workflow (payment/shipping) is independent with no cross-dependencies
- **Dead Letter Queues:** Built-in DLQs for payment and shipping handle failures separately
- **Visibility Timeout:** Different processing times managed per queue (1s payment, 1.5s shipping)
- **Simplicity:** 2 Lambda consumers are easier to reason about than 2 SNS topics
- **Cost:** SQS is more cost-effective for this use case (predictable, low volume)
- **Testing:** Easier to mock and test with predictable queue behavior

**When You Might Use SNS:**
- Multiple services need the same event (fan-out to 3+ subscribers)
- Real-time notifications to external systems
- Complex routing rules needed

### Why DynamoDB?

- **PAY_PER_REQUEST billing:** Perfect for variable order volume
- **Simple schema:** Orders with string attributes
- **Single partition key:** Fast lookup by orderId
- **TTL available:** Could auto-expire test data

## Project Structure

```
order-processing-db-test/
├── README.md
├── package.json
├── tsconfig.json
├── jest.config.js
